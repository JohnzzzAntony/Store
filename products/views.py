"""
products/views.py
CRUD for products using Django models.
Admin → full write access  |  Public → read-only
"""
...
def _validate_product(data):
    ...
    if not name or len(name) > 200:  # adjust to model max
...
def list_products(request):
    ...
    Query params:
      ...
      ?search=<term>     name contains term (case-insensitive)
    """
    ...
    for product in products_qs:
        data = {
            ...
            "stock": 1 if product.in_stock else 0,  # approximate
...
def update_product(request, product_id):
    ...
    product.in_stock = int(request.data["stock"]) > 0
    product.is_featured = bool(request.data.get("featured", False))
    product.save()
...
def patch_product(request, product_id):
    """Partial update — useful for stock adjustment, toggling featured, etc."""
    try:
        product = Product.objects.get(id=product_id)
    except Product.DoesNotExist:
        return Response({"error": "Product not found."}, status=404)

    allowed = {"name", "description", "price", "category", "stock", "featured"}
    ...
    if "description" in patch:
        product.description = patch["description"].strip()
    if "price" in patch:
        product.price = float(patch["price"])
    if "category" in patch:
        try:
            category = Category.objects.get(slug=patch["category"])
            product.category = category
        except Category.DoesNotExist:
            return Response({"error": "Category not found."}, status=400)
    if "stock" in patch:
        product.in_stock = int(patch["stock"]) > 0
    if "featured" in patch:
        product.is_featured = bool(patch["featured"])
