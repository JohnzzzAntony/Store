...
from store.models import (Product, Customer, Order, OrderItem, ShippingAddress, Category, Brand,
                         BlogPost, ContactMessage, PromoBanner, CategoryOffer, BOGOOffer, FrontendMedia)
...
def store(request, *args, **kwargs):
    ...
    featured_products = Product.objects.filter(store=store_obj, in_stock=True).order_by('-created_at')[:6]
