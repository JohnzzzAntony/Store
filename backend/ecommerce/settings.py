...
if not SECRET_KEY:
    # During development/CI, we might allow a fallback, but per request we raise error
...
if not DEBUG:
    required_keys = ['STRIPE_PUBLIC_KEY', 'STRIPE_SECRET_KEY'] # Webhook secret might be optional initially but recommended
    for key in required_keys:
        if not os.environ.get(key):
            raise ImproperlyConfigured(f"{key} is required for production")

STRIPE_CURRENCY = 'usd'   # change to your currency

STORE_ADMIN_EMAIL = os.environ.get('STORE_ADMIN_EMAIL', 'admin@example.com')
